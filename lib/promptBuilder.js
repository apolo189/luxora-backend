...
// These are phrased as strong, mandatory, standalone commands (repeated
// later in the prompt too) because AI image-editing models can otherwise
// treat furniture changes as optional/secondary compared to surface
// (wall/floor) color changes.
...
function buildPrompt(config, variantId) {
  ...
  const prompt = [
    ...
    furnitureMode !== 'keep' ? `Reminder: the furniture instruction above is mandatory and must be clearly, visibly applied in the output — it is not optional.` : '',
